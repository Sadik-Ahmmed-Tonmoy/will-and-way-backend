...
const globalErrorHandler = (
  ...
  // SAFE LOGGING – avoids calling error's inspect() method
  console.error("=== GLOBAL ERROR ===");
  console.error("Name:", error?.name);
  console.error("Message:", error?.message);
  console.error("Stack:", error?.stack);
  // If you need to log Prisma-specific details, add them conditionally
  ...
  // Prisma Known Request Errors
    ...
    if (error.code === "P2025") {
      ...
      let targetField = "unknown_field";

      if (typeof error.meta?.target === "string") {
        targetField = error.meta.target
          .replace(/^.*_/, "") // remove table prefix if exists
          .replace("_key", "");
      ...
      errorMessages = [
        {
          path: targetField,
          message: `A ${targetField} already exists.`,
        },
      ];
    } else {
      statusCode = httpStatus.BAD_REQUEST;
